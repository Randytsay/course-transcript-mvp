  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <div className={styles.workspace}>
      <section className={styles.videoPanel}>
        ...
        </div></div>{message?<p aria-live="polite" className={styles.muted} role="status">{message}</p>:null}</div>
        ...
        <div aria-labelledby={`learning-tab-${tab}`} className={styles.tabBody} id={`learning-panel-${tab}`} role="tabpanel">
          {tab==="transcript"?<div className={styles.list}>{lesson.segments.map(segment=><button aria-label={`${stamp(segment.start_ms)}，字幕第 ${segment.segment_index} 段：${segment.working_text}`} key={segment.id} onClick={()=>seek(segment.start_ms)} type="button" className={styles.listItem} style={{textAlign:"left",cursor:"pointer",background:active?.id===segment.id?"#edf5f1":undefined}}><strong>{stamp(segment.start_ms)}</strong><p>{segment.working_text}</p></button>)}</div>:null}
