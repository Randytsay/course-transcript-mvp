...
export default function ReviewCenterPage(){
 const [items,setItems]=useState<ReviewItem[]|null>(null);const [error,setError]=useState<string|null>(null);const [loading,setLoading]=useState(true);
 ...
 return <main className={styles.page}><div className={styles.shell}>
  ...
  {error?<div className={styles.error} role="alert"><p>{error}</p><button onClick={()=>void load()} type="button">重新載入</button></div>:loading&&items===null?<p className={styles.status}>正在整理複習清單…</p>:items?.length?<section className={styles.lessonGrid}>{items.map(item=><article className={styles.lessonCard} key={item.youtube_video_id}>
    {/* eslint-disable-next-line @next/next/no-img-element */}<img loading="lazy" decoding="async" className={styles.thumb} src={`https://i.ytimg.com/vi/${item.youtube_video_id}/mqdefault.jpg`} alt={`${item.title} 縮圖`}/>
    <div className={styles.lessonBody}><h3 className={styles.lessonTitle}>{item.title}</h3><div className={styles.metaRow}><span className={`${styles.badge} ${styles.badgeDue}`}>今天要複習</span><span className={styles.badge}>{cadence[Math.min(item.stage||0,cadence.length-1)]}</span>{item.artifact_id?<span className={styles.badge}>AI 複習內容可用</span>:<span className={styles.badge}>可回看影片與筆記</span>}</div><p className={styles.muted}>上次影片位置 {stamp(item.last_playback_ms)}{item.completed_cycles?` ・ 已完成 ${item.completed_cycles} 輪複習`:""}</p><div className={styles.cardActions}><a className={styles.openButton} href={`/review/learn/${item.youtube_video_id}?tab=review`}>{item.artifact_id?"開始快速複習":"回到課程複習"}</a><a className={styles.reviewButton} href={`/review/learn/${item.youtube_video_id}?t=${Math.floor((item.last_playback_ms||0)/1000)}`}>回看影片</a></div></div>
  </article>)}</section>:<div className={styles.empty}>今天沒有到期的複習。可以回到學習中心繼續下一堂，也可以自由重看已完成課程。</div>}
