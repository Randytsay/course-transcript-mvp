...
export default function JobDetailPage({ jobId }: { jobId: string }) {
  ...
  const publishLabel = job.drivePublished ? "已輸出至原始資料夾" : job.drivePublicationStatus === "pending_retry" ? "輸出待回寫" : "QA 完成後自動輸出";
  return <AppShell title={job.course} description={`${job.filename} · ${job.duration} · ${job.language}`} actions={<><Link href="/" className="button button--ghost"><ArrowLeft size={17} />返回</Link>{pausable && <button className="button button--ghost" disabled={jobAction !== null} onClick={() => void control("pause")}><Pause size={16} />暫停</button>}{job.status === "paused" && <button className="button button--primary" disabled={jobAction !== null} onClick={() => void control("resume")}><Play size={16} />繼續</button>}{job.status === "failed" && job.activeStage && <button className="button button--primary" disabled={jobAction !== null} onClick={() => void control("retry")}><RotateCcw size={16} />重試 {job.activeStage}</button>}<button className="button button--primary" disabled title="只會輸出選定格式；原始音檔不會被修改"><FolderUp size={17} />{publishLabel}</button></>}>
    ...
    <section className="review-workspace"><div className="review-main">
      ...
      <div className="workspace-panel"><div className="workspace-tabs"><button className={tab === "transcript" ? "workspace-tab workspace-tab--active" : "workspace-tab"} onClick={() => setTab("transcript")}>正式逐字稿</button><button className={tab === "qa" ? "workspace-tab workspace-tab--active" : "workspace-tab"} onClick={() => setTab("qa")}>QA 報告</button><button className={tab === "files" ? "workspace-tab workspace-tab--active" : "workspace-tab"} onClick={() => setTab("files")}>輸出檔案</button></div>
        ...
        {tab === "qa" && <div className="qa-grid"><article className="qa-card"><Gauge size={20} /><div><span>目前 QA 狀態</span><strong className={qaStep?.status === "warning" ? "text-warning" : "text-success"}>{qaStep?.detail ?? "尚未產生"}</strong><small>以後端實際 QA 報告為準，不由前端推測。</small></div></article><div className="qa-notice"><CheckCircle2 size={19} /><div><strong>{job.drivePublished ? "已輸出至原始資料夾" : job.drivePublicationStatus === "pending_retry" ? "Drive 暫時限流，輸出待回寫" : "通過 QA 後會自動輸出"}</strong><p>僅輸出你建立任務時勾選的格式；原始 Drive 音檔不會被修改，詞彙審查仍可繼續。</p></div></div><article className="qa-card"><Clock3 size={20} /><div><span>階段與錯誤紀錄</span><strong>{events.length} 筆事件</strong><small>{events.slice(0, 8).map((event) => `${event.createdAt} · ${event.eventType}`).join("｜") || "尚無事件"}</small></div></article></div>}
