...
if retention_env.get("GOOGLE_APPLICATION_CREDENTIALS") != "/run/ai-runtime/gcp-sa.json":
    raise SystemExit("retention monitor must use the mounted AI runtime credential path")
retention_runtime = next(
    (item for item in retention.get("volumes", []) if item.get("target") == "/run/ai-runtime"),
    None,
)
if not retention_runtime or not bool(retention_runtime.get("read_only")):
    raise SystemExit("retention monitor must mount AI runtime read-only")
