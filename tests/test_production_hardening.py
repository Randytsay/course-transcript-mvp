...
class ProductionHardeningTests(unittest.TestCase):
    def test_recovery_schedule_enforces_poll_and_backoff(self) -> None:
        from app.pipeline.recovery_schedule import is_due, load, schedule
    ...
    def test_content_guard_falls_back_only_for_severe_drift(self) -> None:
        ...
        self.assertIn(
            "excessive_deletion",
            content_guard("這是一段相當長而且需要完整保留意思的逐字稿內容", "摘要"),
        )
    ...
    def test_malformed_parent_gemini_response_is_persisted_before_split(self) -> None:
        ...
        items = [
            {"segment_id": "a", "start_ms": 0, "end_ms": 1000, "raw_text": "甲"},
            {"segment_id": "b", "start_ms": 1000, "end_ms": 2000, "raw_text": "乙"},
        ]
        calls = 0
        ...
        def fake_generate(prompt: str, schema: dict[str, object]):
            ...
            if calls == 1:
                text = json.dumps(
                    {"segments": [{"segment_id": "a", "corrected_text": "甲", "uncertain_terms": []}]}
                )
            else:
                segment_id = "a" if calls == 2 else "b"
                text = json.dumps(
                    {"segments": [{"segment_id": segment_id, "corrected_text": segment_id, "uncertain_terms": []}]}
                )
        ...
        with tempfile.TemporaryDirectory() as temp, patch.object(
            hardened.base, "WORK", Path(temp)
        ), patch.object(hardened, "generate_json", side_effect=fake_generate):
    ...
    def test_srt_import_rejects_partial_parse(self) -> None:
