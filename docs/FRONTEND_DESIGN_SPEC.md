...
The frontend should make five questions immediately answerable:

1. Which recording is being processed?
2. Which pipeline stage is currently active?
3. Has the timeline and output passed automated QA?
4. Which terms still require human review?
5. Is the job safe to upload back to Google Drive?
...
- Job metadata and aggregate status
...
- Drive upload action gated by review completion

## Design principles

- **Operational clarity:** status and next action are more important than decorative visuals.
- **Progressive disclosure:** detailed diagnostic information belongs in the job page, not the dashboard.
- **Human review first:** uncertain terminology must remain visible until confirmed or explicitly ignored.
- **No false completion:** `completed`, `review`, and `failed` are visually distinct states.
- **Responsive by default:** desktop is optimized for simultaneous audio and transcript review; mobile stacks the same content without hiding core actions.
...
Spacing follows a practical 4 px base. Main panel radius is 12 px. The sidebar uses a dark neutral tone to separate global navigation from job content.
...
Segment IDs, order, `start_ms`, and `end_ms` remain immutable during text correction.
...
- Dashboard table and right health rail
- Job review workspace uses transcript + 315 px term-review rail

### Tablet 681–1179 px

- Sidebar becomes a drawer below 900 px
- Auxiliary dashboard cards stack or use two columns
...
- Transcript status appears below corrected text
- Search and nonessential keyboard hints are hidden

## Accessibility requirements

- All icon-only controls require `aria-label`.
- Status must never be communicated by color alone.
- Keyboard users must be able to select transcript segments and term actions.
- Focus states must remain visible after component-library adoption.
- Waveform integration must provide text-based current time and duration.
