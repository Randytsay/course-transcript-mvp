...
path. This prevents a release checked out elsewhere from accidentally starting with an
empty database or job directory.

## Mandatory render checks

Before any build or restart, verify the rendered model:
...
is false. Do not clear or rewrite the state merely to make deployment proceed.
