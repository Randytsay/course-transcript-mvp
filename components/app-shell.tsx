    ...
    <div className="app-frame">
      <aside className={`sidebar ${menuOpen ? "sidebar--open" : ""}`}>
        <div className="brand-row">
          ...
          <button className="icon-button sidebar-close" onClick={() => setMenuOpen(false)} aria-label="關閉選單">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="pipeline-card">
          ...
          <div>
            ...
            <span>Chirp、Gemini、GCS 已連線</span>
          </div>
        </div>
      ...
      <main className="main-area">
        <header className="topbar">
          <button className="icon-button mobile-menu" onClick={() => setMenuOpen(true)} aria-label="開啟選單">
            <Menu size={21} />
          </button>
          <div className="search-box">
            <Search size={17} />
            <input aria-label="搜尋任務" placeholder="搜尋檔名、課程或任務編號" />
            <kbd>⌘ K</kbd>
          </div>
