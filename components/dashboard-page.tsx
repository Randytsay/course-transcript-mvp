...
export default function DashboardPage() {
  return (
    <AppShell
      title="轉錄儀表板"
      description="掌握長檔辨識進度、人工審查與輸出狀態。"
      actions={
        ...
        {metrics.map((metric) => {
          ...
          return (
            <article className="metric-card" key={metric.label}>
              <div className={`metric-icon metric-icon--${metric.tone}`}><Icon size={20} /></div>
      ...
      <section className="dashboard-grid">
        <div className="panel panel--jobs" id="jobs">
          <div className="panel-header">
            ...
            <button className="button button--ghost">查看全部 <ArrowRight size={16} /></button>
          </div>

          <div className="jobs-table" role="table" aria-label="最近轉錄任務">
            <div className="jobs-table__header" role="row">
              <span>檔案與課程</span><span>處理進度</span><span>狀態</span><span>更新時間</span><span />
            </div>
            {jobs.map((job) => (
              <div className="jobs-table__row" role="row" key={job.id}>
                <div className="job-file-cell">
                  <div className="file-icon"><FileAudio2 size={20} /></div>
                  <div>
                    <Link href={`/jobs/${job.id}`} className="job-name">{job.filename}</Link>
                    <span>{job.course} · {job.duration}</span>
                  </div>
                </div>
                <div className="job-progress-cell">
                  <ProgressRing value={job.progress} />
                  <span>{job.progress === 100 ? "處理完成" : `${job.progress}%`}</span>
                </div>
                <div><StatusBadge status={job.status} /></div>
                <div className="updated-cell"><Clock3 size={15} />{job.updatedAt}</div>
                <div className="row-action">
                  <Link className="icon-button" href={`/jobs/${job.id}`} aria-label={`查看 ${job.filename}`}>
        ...
        <aside className="dashboard-side">
          <div className="panel health-panel">
            <div className="panel-header panel-header--compact">
              ...
              <span className="health-score">98%</span>
            </div>
          ...
          <div className="panel quick-panel">
            <div className="quick-panel__icon"><Sparkles size={21} /></div>
            <h2>最高精準度模式</h2>
            <p>Chirp 建立時間軸，Gemini 依固定字幕段落進行繁體與術語校正。</p>
            <Link href="/jobs/new" className="button button--secondary">建立精準轉錄 <ArrowRight size={16} /></Link>
          </div>
        </aside>
      </section>
    </AppShell>
